    ...
    public class ManageProvider : IManageProvider
    {
        ...
        public virtual IManageUser Current()
        {
            try
            {
                ...
                if (LoginProvider == "Cookie")
                {
                    user = JsonHelper.JsonToEntity<IManageUser>(DESEncryptHelper.Decrypt(CookieHelper.GetCookie(LoginUserKey)));
                }
                else
                { 
                    user = JsonHelper.JsonToEntity<IManageUser>(DESEncryptHelper.Decrypt(SessionHelper.Get(LoginUserKey).ToString()));
                }
                if (user == null)
                {
